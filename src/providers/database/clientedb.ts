    ...
    if (!this.isOpen) {
      ...
      this.storage.create({ name: "data.db", location: "default" }).then((db: SQLiteObject) => {
        ...
        db.executeSql(
          ...
          "CREATE TABLE IF NOT EXISTS Compra (id INTEGER PRIMARY KEY AUTOINCREMENT, idCliente REFERENCES Cliente (id) NOT NULL, dataCompra DATE NOT NULL, status BOOLEAN DEFAULT (0), Pagamento BOOLEAN DEFAULT (0), primeiraParcela DATE, qtdParcelas INTEGER, total DOUBLE );" +
          "CREATE TABLE IF NOT EXISTS Item (id INTEGER PRIMARY KEY AUTOINCREMENT, Item STRING NOT NULL, Quantidade NUMERIC NOT NULL DEFAULT (1), preco DOUBLE NOT NULL, idCompra INTEGER REFERENCES Compra (id) NOT NULL);"
          , []);
